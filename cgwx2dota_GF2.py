...
def cgwx2dota(tif_dir, shp_dir, label_dir):
    ...
    for tif_name in os.listdir(tif_dir):
        if tif_name.endswith('.tif') != True:
            ...
            img_name = tif_name[:-4]
            name_list = re.split('_|-', img_name)
            # shp_name = img_name.split('_')[0] + '_' + img_name.split('_')[4] + '_' + img_name.split('_')[5].split('-')[0]
            shp_name = name_list[0] + '_' + name_list[4] + '_' + name_list[5]
            shp_name = osp.join(shp_dir, shp_name + '.shp')
...
if __name__ == '__main__':
    # tif_dir
    tif_dir = r"\\192.168.100.52\PersonalWork\黄锵锵\GF2\tif1"
    shp_dir = r"D:\data\GF2\shp"
    label_dir = r"D:\data\GF2\dota"
